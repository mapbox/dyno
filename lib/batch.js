...
items.deleteItems = function(itemKeys, opts, cb) {
    ...
    var deleteRequestSizes = deleteRequests.map(function(d) {
        return itemSize(d.DeleteRequest.Item);
    });
    
    if (!deleteRequestSizes.every(function(pr) {
        return pr < maxSize;
    })) {
        return cb(new Error('a chunk in this deleteItems call is too large.'));
    }
    ...
    for (var i = 0; i < deleteRequestSizes.length; i++) {
        if ((thisChunk + deleteRequestSizes[i]) < maxSize &&
            chunk.length < 25) {
            chunk.push(deleteRequests[i]);
            thisChunk += deleteRequestSizes[i];
        } else {
            chunks.push(chunk);
            chunk = [];
            thisChunk = 0;
    ...
    chunks.forEach(function(deleteRequests) {
        ...
        q.defer(config.dynamo.batchWriteItem.bind(config.dynamo), d);
    });
