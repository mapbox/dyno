...
module.exports = function(client) {
  ...
  function sendCompletely(requests, fnName, maxRetryCount, concurrency, callback) {
    ...
    function sendOne(req, callback) {
      ...
      var result = { error: undefined, data: {}, unprocessed: {} };
      ...
      (function send(req) {
        req.on('complete', function(response) {
          ...
          if (newParams.RequestItems === undefined || Object.keys(newParams.RequestItems).length === 0) {
            return callback(null, result);
          }
          ...
          if (retryCount > maxRetryCount) {
            result.unprocessed = newParams.RequestItems;
            result.error = new Error('Timeout: Items still left to process');
            return callback(null, result);
          }
  ...
  requests.batchWriteAll = function(params, maxRetryCount) {
    maxRetryCount = maxRetryCount || 10;
    var requestSet = requests.batchWriteItemRequests(params);
    requestSet.sendAll = sendCompletely.bind(null, requestSet, 'batchWrite', maxRetryCount);
    return requestSet;
  };

  requests.batchGetAll = function(params, maxRetryCount) {
    maxRetryCount = maxRetryCount || 10;
    var requestSet = requests.batchGetItemRequests(params);
    requestSet.sendAll = sendCompletely.bind(null, requestSet, 'batchGet', maxRetryCount);
    return requestSet;
  };
