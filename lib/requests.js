...
module.exports = function(client) {
   ...
   * @param {function} callback - a function to handle the response
   */
  function sendCompletely(requests, fnName, maxRetires, concurrency, callback) {
    if (typeof concurrency === 'function') {
      callback = concurrency;
      concurrency = 10;
    }
    ...
    function sendOne(req, callback) {
      ...
      (function send(req) {
        ...
        req.on('complete', function(response) {
          ...
          if (newParams.RequestItems && Object.keys(newParams.RequestItems).length && retryCount < maxRetires) {
  ...
  requests.batchWriteAll = function(params, maxRetires) {
    maxRetires = maxRetires || 10;
    var requestSet = requests.batchWriteItemRequests(params);
    requestSet.sendAll = sendCompletely.bind(null, requestSet, 'batchWrite', maxRetires);
    return requestSet;
  };

  requests.batchGetAll = function(params, maxRetires) {
    maxRetires = maxRetires || 10;
    var requestSet = requests.batchGetItemRequests(params);
    requestSet.sendAll = sendCompletely.bind(null, requestSet, 'batchGet', maxRetires);
    return requestSet;
  };
