...
module.exports = function(client) {
  ...
  function sendAll(requests, fnName, concurrency, callback) {
    ...
    requests.forEach(function(req) {
      q.defer(function(next) {
        ...
        // Convert function to promise call
        if (typeof req === 'function') {
          req()
            .then(response => next(null, { error: null, data: response }))
            .catch(err => next(null, { error: err, data: null }));
        } else {
          // Assuming req is already a promise
          req
            .then(response => next(null, { error: null, data: response }))
            .catch(err => next(null, { error: err, data: null }));
        }
    ...
    q.awaitAll(function(err, responses) {
      ...
      unprocessed = unprocessed.map(function(params) {
        if (params) return () => client[fnName](params);
        else return null;
      });
  ...
  function sendCompletely(requests, fnName, maxRetries, concurrency, callback) {
    ...
    function sendOne(req, callback) {
      ...
      (function send(reqPromise, originalParams) {
        if (typeof reqPromise === 'function') {
        ...
        reqPromise
          .then(response => {
            if (response.Responses) {
              ...
              Object.keys(response.Responses).forEach(function(table) {
                result.data.Responses[table] = result.data.Responses[table] || [];
                response.Responses[table].forEach(function(res) {
            ...
            if (response.ConsumedCapacity) {
              ...
              reduceCapacity(result.data.ConsumedCapacity, response.ConsumedCapacity);
            }

            var newParams = {
              RequestItems: response.UnprocessedItems || response.UnprocessedKeys
            };
            
            // Add original request parameters if available
            if (originalParams && originalParams.ReturnConsumedCapacity) {
              newParams.ReturnConsumedCapacity = originalParams.ReturnConsumedCapacity;
            }
            ...
            if (newParams.RequestItems && Object.keys(newParams.RequestItems).length && retryCount < maxRetries) {
              ...
              return setTimeout(function() {
                send(() => client[fnName](newParams), newParams);
              }, delay);
            }

            if (response.UnprocessedKeys) {
              result.data.UnprocessedKeys = result.data.UnprocessedKeys || {};
              Object.keys(response.UnprocessedKeys).forEach(function(table) {
                result.data.UnprocessedKeys[table] = result.data.UnprocessedKeys[table] || { Keys: [] };
                response.UnprocessedKeys[table].Keys.forEach(function(res) {
            ...
            if (response.UnprocessedItems) {
              ...
              Object.keys(response.UnprocessedItems).forEach(function(table) {
                result.data.UnprocessedItems[table] = result.data.UnprocessedItems[table] || [];
                response.UnprocessedItems[table].forEach(function(res) {
      ...
      })(req, req && req.params ? req.params : null);
    }
