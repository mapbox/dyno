...
module.exports.query = function(conditions, opts, cb) {
    ...
    if(opts.filter) {
        for(c in opts.filter) {
            var con = conditions[c];
            var compare = Object.keys(con)[0];
            QueryFilter[c] = {};
            QueryFilter[c].ComparisonOperator = compare;
            QueryFilter[c].AttributeValueList = [];
            if(!Array.isArray(con[compare])) con[compare] = [con[compare]];
            con[compare].forEach(function(avl){
                QueryFilter[c].AttributeValueList.push(types.toDynamoTypes({val:avl}).val);
            });
        }
    }
    ...
    if(opts.capacity) {
        params.ReturnConsumedCapacity = opts.capacity;
    }
    if(opts.forward !== undefined) {
        params.ScanIndexForward = opts.forward;
    }
