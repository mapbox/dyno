...
| ---- | ---- | ----------- |
| `ConsumedCapacity` | `object` | once the stream has begun making requests, the `ConsumedCapacity` parameter will report the total capacity consumed by the aggregate of all requests that have completed. This property will only be present if the `ReturnConsumedCapacity` parameter was set on the initial request. |
...
* `list` **`array`** an array of strings, numbers, or buffers to store in DynamoDB as a set
...
* `concurrency` **`[number]`** the concurrency with which to make requests. Default value is `1`.
